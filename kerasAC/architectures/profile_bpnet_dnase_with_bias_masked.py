...
def load_pretrained_bias(model_hdf5):
    ...
    '''
    for i, layer in enumerate(pretrained_bias_model.layers):
       if i==1:
           for j, layert in enumerate(layer.layers):
               if j !=0 :
                   layert._name = layert.name + 'nbias_layer_' + str(j)
       if i==2:
           for j, layert in enumerate(layer.layers):
               if j !=0 :
                   layert._name = layert.name+ 'nnbias_layer_' + str(j)

    extracted_layers = pretrained_bias_model.layers

    pretrained_bias_model = keras.Sequential(extracted_layers)
    '''
#    model_cloned = tf.keras.models.clone_model(pretrained_bias_model)
#    model_cloned.set_weights(pretrained_bias_model.get_weights())
#    return model_cloned
