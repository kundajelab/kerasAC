...
def fetch_tomtom_matches(ppm, cwm, meme_fname,background=[0.25, 0.25, 0.25, 0.25], tomtom_exec_path='tomtom', motifs_db='HOCOMOCOv11_core_HUMAN_mono_meme_format.meme' ,
    ...
    trim_thresh = np.max(score) * trim_threshold  # Cut off anything less than 20% of max score
    ...
    write_meme_file(trimmed, background, meme_fname)
    
    # run tomtom
    cmd = '%s -no-ssc -oc . -verbosity 1 -text -min-overlap 5 -mi 1 -dist pearson -evalue -thresh 10.0 %s %s' % (tomtom_exec_path, meme_fname, motifs_db)
    #print(cmd)
...
def fetch_args():
    ...
    parser.add_argument("-q", "--q_threshold",type=float,default=0.1,help="TomTom hit q-threshold to report")
...
def main(): 
    ...
    for i,x in enumerate(ppms):
        print(str(i))
        res.append(fetch_tomtom_matches(x, cwms[i],names[i], tomtom_exec_path=args.tomtom_exec, motifs_db=args.meme_motif_db,
    ...
    with open(args.outfile, 'w') as f:
        ...
        for i,r in enumerate(res):
            if len(r)>0:
                ...
                for match in r:
                    if match['q-value']< args.q_threshold: 
